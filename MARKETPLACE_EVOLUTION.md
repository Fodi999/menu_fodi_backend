...
### GET `/api/leaderboard`
**Глобальный рейтинг поваров** по XP, продажам и AI-оценкам.

**Query Parameters:**
- `sortBy` - критерий сортировки: `xp` (default), `sales`, `rating`, `revenue`
- `language` - фильтр по языку: `pl`, `ua`, `en`
- `limit` - количество результатов (1-100, default: 50)

**Examples:**
```bash
# Топ-10 поваров по опыту
GET /api/leaderboard?sortBy=xp&limit=10

# Топ продавцов
GET /api/leaderboard?sortBy=sales

# Лучшие по AI-рейтингу среди польских поваров
...
**Метрики рейтинга:**
- `totalXp` - общий опыт (курсы, рецепты, достижения)
- `totalSales` - количество проданных рецептов
- `totalRevenue` - чистый доход от продаж (после комиссии 10%)
- `averageRating` - средний AI-рейтинг публичных рецептов
- `recipeCount` - количество публичных рецептов
- `achievementCount` - количество разблокированных достижений
...
**Planned Features:**
- [x] **Chef Leaderboard** - GET /api/leaderboard ✅ РЕАЛИЗОВАНО
...
| Chef Leaderboard | GET /api/leaderboard | ✅ | 2 chefs ranked |
